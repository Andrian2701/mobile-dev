...
fun LabDetailsScreen(
    ...
    labId: Int,
    vm: LabDetailsViewModel = koinViewModel()
) {
    val lab by vm.labFlow(labId).collectAsState(initial = null)
    val comments by vm.commentsFlow(labId).collectAsState(initial = emptyList())
    ...
    Scaffold(
        ...
        Column(
            ...
            Button(
                onClick = {
                    vm.addComment(labId, commentInput)
                    commentInput = ""
                },
                modifier = Modifier.fillMaxWidth()
            ) {
